## Friday — Pull + Micro Push

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| **Back** | | | |
| Low Anchor Row | Lower lats / Mid back | 🟣🟡 | 3 × 6 |
| Straight Arm Pulldown | Lats | ⚫🟡 | 3 × 6 |
| Straight Row | Mid traps / Rhomboids | 🟣🔴 | 3 × 6 |
| Face Pulls | Rear delts | 🟡 | 2 × 12 |
| ─ | ─ | ─ | ─ |
| **Biceps (heavy/stretch)** | | | |
| Behind the Back Cable Curl | Long head | ⚫ | 3 × 6 |
| Overhand Curl | Brachialis | 🟣🔴 | 3 × 6 |
| ─ | ─ | ─ | ─ |
| **Micro Add-On (light)** | | | |
| Push Downs | Lateral / Medial heads | 🔴 | 2 × 12 |
| Side Lateral Raise | Side delt | 🟡| 2 × 12 |

## Saturday — Push

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| **Chest** | | | |
| Upper Chest Forward Press | Upper chest | 🟣 | 3 × 8 |
| Fly | Chest | 🟣 | 3 × 10 |
| High Crossover Down | Lower pec | ⚫ | 3 × 12 |
| ─ | ─ | ─ | ─ |
| **Shoulders** | | | |
| Band Pull-Aparts | Rear delts / Posture | 🔴🟡 | 2 × 20 |
| Military Press | Front delt | 🟣🟡 | 3 × 6 |
| Side Lateral Raise | Side delt | 🔴 | 3 × 6 |
| ─ | ─ | ─ | ─ |
| **Triceps** | | | |
| Overhead Extension | Long head | ⚫🔴 | 3 × 6 |
| Push Downs | Lateral / Medial heads | ⚫ | 3 × 6 |
| Skull Crushers | Lateral / Long heads | 🔴 | 3 × 8 |


<div style="page-break-after: always;"></div>

## Sunday — Legs + Biceps

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| **Legs** | | | |
| Overhead Squats | Quads / Glutes / Hammies | 🟣 | 3 × 8 |
| Conventional Deadlift | Glutes / Hammies / Quads | 🟣🟡 | 3 × 8 |
| Split Squat | Quads/Glutes | 🟣 | 2 × 8 |
| Squatted Side Steps | Glutes | ⚫🔴 | 2 × 10 |
| Leg Curls | Hamstrings | 🟣 | 2 × 12 |
| Calf Raises | Calves | 🟣 | 2 × 15–20 |
| ─ | ─ | ─ | ─ |
| **Biceps (pump)** | | | |
| Underhand Curl | Inner bicep | 🟣 | 2 × 12 |
| Drag Curls | Peak bicep | 🔴🟡 | 2 × 12 |
| One Arm Cross Body Curl | Brachialis | 🔴 | 2 × 12 |
| ─ | ─ | ─ | ─ |
| **Core** | | | |
| Pallof Press | Core | 🟣 | 2 × 10 |

## Mid Week — Micro Arms/Chest

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| Standing Chest Press/Fly | Chest | 🟣| 3 × 15 |
| Side Lateral Raise | Side delt | 🔴/🟡| 2–3 × 12 |
| Tricep Overhead Extension | Long head | 🔴 | 2–3 × 12 |
| Behind the Back Cable Curl | Long head | 🔴 | 2-3 × 12 |

## Mid Week — Micro Legs/Back/Core

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| Straight Row | Mid traps / Rhomboids | 🔴🟡 | 2 × 12 |
| Face Pulls | Rear delts | 🟡 | 2 × 12 |
| Spanish Squats | Quads | 🟢 | 2 × 12 |
| Band Pull-Through | Glutes / Hammies | ⚫ | 2 × 12 |
| Calf Raises | Calves | 🟣 | 2 × 15-20 |
| Ab Wheel Rollout | Core | — | 2 × 8 |
