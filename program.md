## Friday — Pull + Micro Push

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| **Back** | | | |
| Low Anchor Row | Lower lats / Mid back | 🟣🟡 | 3 × 6–8 |
| Single Arm Pulldown | Lats | 🟣 | 3 × 10–12 |
| Straight Row | Mid traps / Rhomboids | 🟣🟡 | 2 × 8–10 |
| Face Pulls | Rear delts | 🟡 | 2 × 12–15 |
| ─ | ─ | ─ | ─ |
| **Biceps (heavy/stretch)** | | | |
| Behind the Back Cable Curl | Long head | ⚫ | 3 × 6–8 |
| Overhand Curl | Brachialis | 🟣🟡 | 2 × 8–10 |
| ─ | ─ | ─ | ─ |
| **Micro Add-On (light)** | | | |
| Push Downs | Lateral / Medial heads | 🔴🟡 | 2 × 12–15 |
| Side Lateral Raise | Side delt | 🟡| 2 × 12–15 |

## Saturday — Push

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| **Chest** | | | |
| Upper Chest Forward Press | Upper chest | 🟣 | 3 × 8–10 |
| Fly | Chest | 🟣 | 3 × 10–12 |
| High Crossover Down | Lower pec | ⚫ | 3 × 10–12 |
| ─ | ─ | ─ | ─ |
| **Shoulders** | | | |
| Band Pull-Aparts | Rear delts / Posture | 🔴🟡 | 2 × 15–20 |
| Military Press | Front delt | 🟣🟡 | 3 × 6–8 |
| Side Lateral Raise | Side delt | 🔴 | 2 × 8–12 |
| ─ | ─ | ─ | ─ |
| **Triceps** | | | |
| Overhead Extension | Long head | ⚫🔴 | 2 × 8–10 |
| Push Downs | Lateral / Medial heads | ⚫ | 2 × 8–12 |
| Skull Crushers | Lateral / Long heads | 🔴 | 3 × 8–10 |


<div style="page-break-after: always;"></div>

## Sunday — Legs + Biceps

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| **Legs** | | | |
| Overhead Squats | Quads / Glutes / Hammies | 🟣 | 3 × 8–10 |
| Conventional Deadlift | Glutes / Hammies / Quads | 🟣🟡 | 3 × 6–8 |
| Split Squat | Quads/Glutes | 🟣 | 2 × 8–10 |
| Squatted Side Steps | Glutes | ⚫🔴 | 2 × 10–15 |
| Leg Curls | Hamstrings | 🟣 | 2 × 10–15 |
| Calf Raises | Calves | 🟣 | 2 × 15–20 |
| ─ | ─ | ─ | ─ |
| **Biceps (pump)** | | | |
| Underhand Curl | Inner bicep | 🟣 | 2 × 10–12 |
| Drag Curls | Peak bicep | 🔴🟡 | 2 × 10–12 |
| One Arm Cross Body Curl | Brachialis | 🔴 | 2 × 10–12 |
| ─ | ─ | ─ | ─ |
| **Core** | | | |
| Pallof Press | Core | 🟣 | 2 × 10–12 |

## Mid Week — Micro Arms/Chest

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| Standing Chest Press/Fly | Chest | 🟣| 2 × 15–20 |
| Side Lateral Raise | Side delt | 🔴/🟡| 2 × 12–15 |
| Tricep Overhead Extension | Long head | 🔴 | 2 × 12–15 |
| Behind the Back Cable Curl | Long head | 🔴 | 2 × 12–15 |

## Mid Week — Micro Legs/Back/Core

| Workout | Muscles Hit | Band | Sets × Reps |
|----------|-------------|------|:---------:|
| Straight Row | Mid traps / Rhomboids | ⚫ | 2 × 12–15 |
| Face Pulls | Rear delts | 🟡 | 2 × 12–15 |
| Spanish Squats | Quads | 🟢 | 2 × 12–15 |
| Band Pull-Through | Glutes / Hammies | 🟢 | 2 × 12–15 |
| Calf Raises | Calves | 🟣 | 2 × 15-20 |
| Ab Wheel Rollout | Core | — | 2 × 8–12 |
